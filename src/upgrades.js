...
function betterBeans(){
    var cost = Math.floor(150 * Math.pow(beansLevel - 1, 1.4));                             //formula for currrent price of better beans
    if(credits >= cost){
        beansLevel++;
        increaseCredits(-cost);
        document.getElementById("betterBeansButton").innerHTML = "Better Beans (" + Math.floor(150 * Math.pow(beansLevel - 1, 1.4)) + " credits)";
        updateCPS();
    }
}
